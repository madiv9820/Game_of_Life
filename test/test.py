...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        board: List[List[int]] = testcase['input']
        ...
        print('🧩 Matrix    :')
        print(_format_matrix(board))
        print('⏳ Starting soon...')
        time.sleep(self.DISPLAY_DELAY_SECONDS)

        try:
            self._run_case(board=board)
        except TimeoutError:
        ...
        if board == expected_output:
            print(f'✅ Result    : Passed with answer {board}')
        else:
            print(f'❌ Result    : Expected {expected_output}, got {board}')
...
class TestSolution(unittest.TestCase):
    ...
    @timeout(1)
    def _run_case(self, board: List[List[int]]) -> List[int]:
        # ⚡ Give every testcase its own timeout instead of timing the whole file.
        return self.__solution.gameOfLife(board=board)


class GameofLifeTextTestResult(unittest.TextTestResult):
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=GameofLifeTextTestResult)
    )
